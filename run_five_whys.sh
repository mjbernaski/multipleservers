...
# Find processes running five_whys_parallel.py
EXISTING_PIDS=$(pgrep -f "five_whys_parallel.py" 2>/dev/null)
...
echo "Starting 5 Whys Analysis server..."
echo "URL: $URL"

# Run the Python script in the background
python five_whys_parallel.py --host "$HOST" --port "$PORT" &
SERVER_PID=$!
